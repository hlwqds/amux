...
pub fn discover_sessions_by_ids(workspaces: &[Workspace]) -> Vec<Session> {
    ...
    for ws in workspaces {
        for sid in &ws.session_ids {
            ...
            if let Some(jsonl_path) = crate::discovery_walk::find_session_jsonl_by_id(sid, &ws_path)
            {
...
pub fn discover_sessions_cached(
    ...
    crate::discovery_walk::collect_claude_jsonl(workspaces, &mut jsonl_files);
    crate::discovery_walk::collect_codex_jsonl(workspaces, &mut jsonl_files);
    crate::discovery_walk::collect_omp_jsonl(workspaces, &mut jsonl_files);
...
pub fn find_session_jsonl(session: &Session) -> Option<PathBuf> {
    match session.agent {
        ...
        Agent::Codex => {
            ...
            crate::discovery_walk::walk_codex_jsonl(&sessions_root, &session.id)
        }
        Agent::Omp => {
            let sessions_root = Agent::Omp.sessions_dir()?;
            crate::discovery_walk::walk_omp_jsonl(&sessions_root, &session.id)
        }
    }
}

/// Parse a session from a JSONL file path, determining the agent type from the path structure.
pub(crate) fn parse_session_from_path(path: &Path, workspaces: &[Workspace]) -> Option<Session> {
...
pub use crate::discovery_walk::find_recent_session_for_workspace;
