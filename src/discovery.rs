        ...
        for subdir in subdirs.flatten() {
            ...
            if let Ok(files) = fs::read_dir(subdir.path()) {
                for file in files.flatten() {
                    ...
                    // Match session to workspace via cwd from JSONL header.
                    // Skip sessions with no cwd — we cannot reliably determine
                    // the workspace from the directory name alone (hyphens in
                    // paths like /home/my-project would be misread).
...
fn discover_omp_sessions(workspaces: &[Workspace], out: &mut Vec<Session>) {
    ...
    // OMP sessions: ~/.omp/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
    if let Ok(subdirs) = fs::read_dir(&sessions_root) {
        for subdir in subdirs.flatten() {
            if !subdir.path().is_dir() {
                continue;
            ...
            if let Ok(files) = fs::read_dir(subdir.path()) {
                for file in files.flatten() {
                    ...
                    // OMP uses the same JSONL v3 format as GSD
                    let meta = parse_gsd_session(&path);
                    let (id, title, cwd) = match meta {
                        Some(m) => m,
                        None => continue,
